const TEXT = {
  ...
  no_referrer:
    "Please try to access this map on https://galapagosplasticfree.nl/ instead ;)",
};

const DATA_URL = "https://plasticadrift.science.uu.nl/galapagosdrifters/";
...
class DrifterApp {
  constructor(center, zoom) {
    ...
    let playControl;
    [this.playButton, playControl] = this.createOLButton(
      TEXT.animation_start,
      this.toggleAnimate.bind(this)
    );
    ...
    controls.push(
      this.createOLButton(
        TEXT.select_drifter,
        this.createSearchModal.bind(this)
      )[1]
    );
  ...
  updateDate(timestamp) {
    ...
    document.getElementById("date").innerHTML =
      TEXT.drifter_date + " " + date.toLocaleDateString("en-UK", options);
  }
  ...
  svgAlphaFix(colour, opacity) {
    if (opacity < 1.0) {
      // svgs have a quirky interaction with rgba opacity, so avoid that
  ...
  click(evt) {
    let pixel = evt.pixel;
    let feature = this.map.forEachFeatureAtPixel(pixel, function (f) {
      return f;
    });
  ...
  showTooltip(drifterName) {
    ...
    if (hourdiff > 24) {
      this.content.innerHTML = `<p><b>Name:</b> ${name}<br><b>Coordinates:</b> ${hdms}<br><b>Last contact:</b> ${Math.floor(
        hourdiff / 24
      )} days ago`;
    } else if (hourdiff > 9) {
      // nine hours 'grace period'
  ...
  zoomToRelevant() {
    let relevant = this.selected.length
      ? this.selected
      : Object.keys(this.data);
    let relevantData = relevant.map((n) => this.data[n]);

    function select1(prev, curr) {
      // select min/max lng/lat from trail
      return [
        Math.min(prev[0], curr[1]),
        Math.min(prev[1], curr[2]),
        Math.max(prev[2], curr[1]),
        Math.max(prev[3], curr[2]),
      ];
    }

    function select4(prev, curr) {
      // select min/max lng/lat from Array<Array[4]> of min/max lng/lat
      return [
    ...
    let bounds = relevantData
      .map((d) => d.reduce(select1, def))
      .reduce(select4);
  ...
  copyShareableURL(e) {
    navigator.clipboard
      .writeText(this.generateQueryURL(false))
      .catch(this.showCopyModal.bind(this));
  }
  ...
  setupSocialButtons() {
    $(".twitter")[0].onclick = (e) =>
      window.open(
        `https://twitter.com/share?url=${this.generateQueryURL(true)}`
      );
    $(".linkedin")[0].onclick = (e) =>
      window.open(
        `https://www.linkedin.com/shareArticle?mini=true&url=${this.generateQueryURL(
          true
        )}`
      );
    $(".facebook")[0].onclick = (e) =>
      window.open(
        `https://www.facebook.com/sharer.php?u=${this.generateQueryURL(
          true
        )}&t=${this.generateQueryURL(true)}`
      );
